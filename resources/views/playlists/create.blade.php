<x-app-layout>
    <div class="max-w-5xl mx-auto pt-16 pb-20 px-8 bg-white rounded-lg shadow-lg min-h-screen">
        ...
        <form action="{{ isset($playlist) ? route('playlists.update', $playlist->id) : route('playlists.store') }}" method="POST" class="space-y-8">
            ...
            <section class="space-y-2">
                <label for="duration" class="block text-lg font-semibold text-gray-700">Duração Padrão de Exibição (segundos)</label>
                <input type="number" name="duration" id="duration" min="1" max="3600" step="1"
                    value="{{ old('duration', $playlist->duration ?? 10) }}"
